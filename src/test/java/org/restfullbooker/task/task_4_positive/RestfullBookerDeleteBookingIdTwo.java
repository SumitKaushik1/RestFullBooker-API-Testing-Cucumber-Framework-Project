...
public class RestfullBookerDeleteBookingIdTwo {
    ...
    private  final  String DELETE_BOOKING_ID;


    public RestfullBookerDeleteBookingIdTwo() {
        ...
        this.DELETE_BOOKING_ID = RestFullBookerCreateBookingOne.getDeletedBookingId();
    }


    @Given("delete bookingid  urlEleven {string}")
    public void delete_bookingid_url_eleven(String baseUri) {
        ...
        requestSpecification.baseUri(baseUri+"/"+DELETE_BOOKING_ID);
       // requestSpecification.basePath("/booking/"+DELETE_BOOKING_ID);
    ...
    @Then("verify responseEleven status code {string}")
    public void verify_response_eleven_status_code(String statusCode) {
        MatcherAssert.assertThat(response.getStatusCode(), Matchers.is(Integer.parseInt(statusCode)));
        //deifnitly it will give bug bz in documents 201 is there but on the delete http response must be 204
        //no content
    }
    @Then("verify responseEleven contains header content type")
    public void verify_response_eleven_contains_header_content_type() {
