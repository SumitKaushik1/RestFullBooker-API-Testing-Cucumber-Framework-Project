...
// private bz i want to intialize the response in this classs i donot want ot share that resposne accross
    ...
    private static String  BOOKING_ID_TO_DELETE;

    // it must be used within the class only so private

    //i donot want the other classes to change the value whatever come in resposne in below code
    public static String getBookingIdToDelete() {
        return  BOOKING_ID_TO_DELETE;
    }


    @Given("Get payload from urlfourteen {string}")
    public void get_payload_from_url_fourteen(String baseUri) {
    ...
    @Then("verify responsefourteen status code \"{int}")
    public void verify_response_fourteen_status_code(Integer int1) {
        // Write code here that turns the phrase above into concrete actions
        MatcherAssert.assertThat(response.getStatusCode(), Matchers.is(Integer.parseInt(String.valueOf(int1))));
    }
    @Then("verify responsefourteen contains header content type")
    public void verify_response_fourteen_contains_header_content_type() {
        // Write code here that turns the phrase above into concrete actions
        MatcherAssert.assertThat(response.getHeaders().hasHeaderWithName("Content-type"),Matchers.is(true));
    }
    @Then("verify responsefourteen contains bookingid")
    public void verify_response_fourteen_contains_bookingid() {
        // Write code here that turns the phrase above into concrete actions
      ...
      //  Response response= RestfullBookerRequestGetAllBookingURL.response;
        ...
        BOOKING_ID_TO_DELETE=jsonPath.getString("[0].bookingid");
    ...
    @Then("verify responsefourteen Json Schema")
    public void verify_response_fourteenseven_json_schema() {
        // Write code here that turns the phrase above into concrete actions
